...
```{r process data, cache = TRUE, warning = FALSE}
library(stringr)
library(lubridate)
...
```{r year-event summary}
library(reshape2)
## melt the data to set the course for summarizine
year.melt <- melt(analysis.data, id = c("year", "event"))
...
<figure>
```{r year-event boxplots, fig.width = 15, fig.height = 20}
library(ggplot2)
library(gridExtra)
...
<figure>
```{r year-event bar plots, fig.width = 15, fig.height = 20}
health.bar <- ggplot(year.cast, aes(x = event, y = health.total)) +
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1, lineheight = 0.7), 
            axis.text.y = element_text(lineheight = 0.7))
econ.bar <- ggplot(year.cast, aes(x = event, y = econ.total)) +
      geom_bar(stat = "identity", alpha = 0.75) + 
